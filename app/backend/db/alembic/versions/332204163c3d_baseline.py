...
def upgrade():
    ...
    sa.Column('type', sa.Enum('required', 'not_required', 'not_important', name='date_type'), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_date_details_date'), 'date_details', ['date'], unique=True)
    ...
    sa.Column('type', sa.Enum('int', 'str', 'bool', 'time', 'date', name='types'), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('value', sa.String(), nullable=True),
    sa.Column('type', sa.Enum('int', 'str', 'bool', 'time', 'date', name='types'), nullable=False),
    sa.ForeignKeyConstraint(['mador_name'], ['madors.name'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_mador_settings_mador_name'), 'mador_settings', ['mador_name'], unique=False)
    ...
    sa.Column('type', sa.Enum('facebook', 'google', 'local', name='user_type'), nullable=True),
    ...
    sa.Column('state', sa.Enum('here', 'not_here', name='answer_state'), nullable=True),
    sa.Column('reason', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['date'], ['date_details.date'], ondelete='CASCADE'),
    ...
    op.create_index(op.f('ix_permissions_map_permission_id'), 'permissions_map', ['permission_id'], unique=False)
    op.create_index(op.f('ix_permissions_map_user_id'), 'permissions_map', ['user_id'], unique=False)
...
def downgrade():
    ...
    op.drop_table('users')
    op.drop_index(op.f('ix_mador_settings_mador_name'), table_name='mador_settings')
