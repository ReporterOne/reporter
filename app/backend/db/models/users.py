...
from sqlalchemy import Integer, String, Column, ForeignKey, Date, Enum, Table, select
...
operators_map = Table('operators_map', Base.metadata,
    Column('mador_id', Integer, ForeignKey('madors.id', ondelete='CASCADE'), index=True),
...
class User(Base):
    ...
    operates = relationship('Mador', secondary=operators_map, backref='operators')
