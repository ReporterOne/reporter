...
class DateData(Base):
    ...
    id = Column(Integer, primary_key=True, unique=True)
    date = Column(Date, ForeignKey('date_details.date', ondelete='CASCADE'),
                  index=True)
...
class RepetativeData(Base):
    ...
    from_date_id = Column(Integer,
                          ForeignKey('date_details.id', ondelete='CASCADE'),
                          index=True)
    from_date = relationship('DateDetails', foreign_keys=[from_date_id])

    to_date_id = Column(Integer,
                        ForeignKey('date_details.id', ondelete='CASCADE'),
                        index=True)
    to_date = relationship('DateDetails', foreign_keys=[to_date_id])

    # bitmask of the week days 1001000 is every sunday and wednesday
    every = Column(Integer)

    reason_id = Column(Integer, ForeignKey('reasons.id'), index=True)
    reason = relationship('Reason', backref='repetative_datas',
                          foreign_keys=[reason_id])
    reported_time = Column(DateTime)
